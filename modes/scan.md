...
Levels are additive — run all, mix results, deduplicate.
...
6b. **Filter by location** using `location_filter` from `portals.yml`:
   **CRITICAL: "remote" alone is not enough — the candidate is in South Africa (Johannesburg, UTC+2) and can only apply to roles that accept SA candidates.**

   For each offer that passes the title filter:
   - If the URL is from a company known as "globally remote" on Greenhouse/Ashby → pass directly
   - If not: WebFetch the JD and check:
     a. If any keyword from `location_filter.negative` appears → record as `skipped_location` and do NOT add to pipeline
     b. If any keyword from `location_filter.positive` appears → pass
     c. If location can't be determined (JD doesn't mention a specific location) → add to pipeline with note `[⚠️ verify SA eligibility]`

   **Signals that SA remote is OK:**
   - "South Africa", "EMEA", "globally remote", "work from anywhere", "worldwide", "Africa", "any location"
   - Company known as remote-first global

   **Exclusion signals (auto-skip):**
   - "Remote - USA", "US only", "must be located in", "US work authorization", "Americas only"
   - Clearly territorial role: "APAC", "North America", "LATAM" without mention of other regions
...
8. **For each new offer that passes filters**:
   a. Add to `pipeline.md` "Pending" section: `- [ ] {url} | {company} | {title}`
      If SA eligibility not confirmed: `- [ ] {url} | {company} | {title} [⚠️ verify SA eligibility]`
...
11. **Offers filtered by location**: record with status `skipped_location`

## Title and company extraction from WebSearch results

Results come in format: `"Job Title @ Company"` or `"Job Title | Company"` or `"Job Title — Company"`.
...
Filtered by location (not SA-eligible): N skipped
Unverified (⚠️ verify SA eligibility): N flagged
Duplicates: N (already evaluated or in pipeline)
New offers added to pipeline.md: N

  + {company} | {title} | {location signal}
  ...
  ⚠️ {company} | {title} | SA eligibility unconfirmed
...
Every company in `tracked_companies` must have `careers_url` — the direct URL to their job listings page. This avoids having to find it each time.
